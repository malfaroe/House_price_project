...
class LASSO(object):
    def __init__(self, dataset, n_iter, alpha, lmbd):
    ...
    def costo(self, h, y, betas):#modificado aquí
        m = len(y) 
        sub = h -y
        cost = np.sum(sub**2)/ (2 * m) + self.lmbd * sum(np.abs(betas)) ##Modificado
        return cost 
    ...
    def theta_update(self, h, betas, X, y):
        m = len(y)
        loss = h -y
        betas = betas - (self.alpha/m) * np.dot(X.T,loss) - self.lmbd  ##modificar
        return betas
...
#         self.dataset = self.normalizer()
        ...
        #updating thetas
            ...
            theta.append(betas)
            cost = self.costo(h, y_train, betas)
        ...
        if summary:
            ...
            print("Root Mean Square Error: %.4f"% np.sqrt(np.round(self.MSE(betas_final, X_test, y_test), decimals = 4))) 
